...
namespace AdventOfCode2020.Days
{
	...
	internal class Day02
	{
		...
		internal static int Part1()
		{
			...
			foreach (string databaseEntry in puzzleInput)
			{
				string[] splittedValue = databaseEntry.Split(':');

				string password = splittedValue[1].Trim();
				string validation = splittedValue[0];

				char validationLetter = validation.Last().ToString().ToCharArray()[0];
				var minimumCount = 0;
				var maximumCount = 0;

				string[] counts = validation.Replace(validation.Last().ToString(), "").Trim().Split('-');
				minimumCount = int.Parse(counts[0]);
				maximumCount = int.Parse(counts[1]);

				int actualCount = password.Count(x => x == validationLetter);
		...
		internal static int Part2()
		{
			...
			foreach (string databaseEntry in puzzleInput)
			{
				...
				string[] positions = validation.Replace(validation.Last().ToString(), "").Trim().Split('-');

				int positionOne = int.Parse(positions[0]);
				int positionTwo = int.Parse(positions[1]);

				if (password.ToCharArray()[positionOne - 1] == validationLetter && password.ToCharArray()[positionTwo - 1] != validationLetter
					|| password.ToCharArray()[positionOne - 1] != validationLetter && password.ToCharArray()[positionTwo - 1] == validationLetter)
				{
					correctPasswords++;
				}
			}
