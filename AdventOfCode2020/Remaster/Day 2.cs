...
namespace AdventOfCode2020.Remaster
{
	using System.Linq;

	internal class PasswordPolicy
	{
		internal PasswordPolicy(string input)
		{
			// "5-6 p: qpppvzp"
			string[] validatorsAndPassword = input.Split(':');

			Password = validatorsAndPassword[1].ToCharArray();
			ValidationCharacter = validatorsAndPassword[0].Last();

			string[] numberString = input.Substring(0, input.IndexOf(' ')).Split('-');

			FirstNumber = int.Parse(numberString[0]);
			SecondNumber = int.Parse(numberString[1]);
		}
		...
		internal char ValidationCharacter { get; }
	...
	internal class Day2
	{
		...
		internal static int Part1()
		{
			...
			{
				var input = new PasswordPolicy(puzzle);

				int occurrences = input.Password.Count(x => x == input.ValidationCharacter);

				if (input.FirstNumber <= occurrences
					&& input.SecondNumber >= occurrences)
				{
					correctPasswords++;
				}
			}
		...
		internal static int Part2()
		{
			...
			foreach (string puzzle in puzzleInput)
			{
				...
				if (input.Password[input.FirstNumber] == input.ValidationCharacter && input.Password[input.SecondNumber] != input.ValidationCharacter
					|| input.Password[input.FirstNumber] != input.ValidationCharacter && input.Password[input.SecondNumber] == input.ValidationCharacter)
				{
					correctPasswords++;
				}
			}
